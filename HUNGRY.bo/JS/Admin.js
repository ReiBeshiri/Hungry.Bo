...
function managesup(){
  $.getJSON("../PHP/Admin.php?request=fornitori", function(data) {
    if(data !== null){
      $("div.admintable").empty();
      var str = "";
      var str1 = '<div class="fluid-container"><div class="row tables"><div class="text-center col-12"><div class="table-header"><p><strong class="searchtable">Fornitori</strong></p></div><table class="table table-hover"><thead class="thead-dark"><tr><th id="username">Username</th><th id="indirizzo">Indirizzo</th><th id="nome-locale">NomeLocale</th><th id="modify">Modify</th><th id="notify">Notify</th><th id="remove" hidden>Remove</th></tr></thead><tbody>';
      var str2 = "";
      var str3 = '</tbody></table><button type="button" class="btn btn-outline-success buttonAdd" data-toggle="modal" data-target="#add-from-admin">Aggiungi</button></div></div></div>';
      for (var i = 0; i < data.length; i++) {
        str2 += '<tr><td headers="username">'+data[i]["Username"]+'</td><td headers="indirizzo">'+data[i]["Indirizzo"]+'</td><td headers="nome-locale">'+data[i]["NomeLocale"]+'</td><td headers="modify"><span class="table-modify"><button type="button" class="btn btn-outline-info buttonModify" data-toggle="modal" data-target="#modify-from-admin">Modify</button></span></td><td headers="notify"><span class="table-modify"><button type="button" class="btn btn-outline-primary buttonNotify" data-toggle="modal" data-target="#send-notify-from-admin">Notify</button></span></td><td headers="remove"><a data-toggle="modal" data-target="#confirm-delete-admin" class="aremove"><input class="cancel" name="cancel" type="image" src="../res/croce.png" alt="delete"/></a></td></tr>';
      }
      str = str1+str2+str3;
      $("div.admintable").append(str)
    }
